...
def movie_synopsys_scrape(titles):
    ...
    try:
        df_movie_synopsis_already_scraped = pd.read_csv(data_folder+'movie_synopsys.csv')
        titles_scraped = list(df_movie_synopsis_already_scraped['title_id'])
        del df_movie_synopsis_already_scraped
    except:
        titles_scraped = []
    ...
    for title_id in titles:
        if titles_scraped.count(title_id) == 0:
            ...
            if i%100 == 0:
                if len(plots) > 0:
                    try:
                        df_movie_synopsys = pd.read_csv(data_folder+'movie_synopsys.csv')
                    except:
                        df_movie_synopsys = pd.DataFrame()

                    df_movie_synopsys = df_movie_synopsys.append(plots)
                    df_movie_synopsys.to_csv(data_folder+'movie_synopsys.csv', index=False)
                    del df_movie_synopsys
                    plots = []
            i += 1
        else:
            j += 1
    ...
    #################################################################################################################################################################################
                                        ...
                                        #To clean above scraped data (movie_synopsys.csv)
