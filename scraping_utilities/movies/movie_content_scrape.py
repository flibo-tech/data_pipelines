...
def movie_content_scrape(titles):
    ...
    try:
        df_movie_content_already_scraped = pd.read_csv(data_folder+'movie_content.csv')
        titles_scraped = list(df_movie_content_already_scraped['title_id'])
        del df_movie_content_already_scraped
    except:
        titles_scraped = []
    ...
    for title_id in titles:
        if titles_scraped.count(title_id) == 0:
            try:
                ...
                try:
                    movie_name = driver.find_element_by_class_name('subpage_title_block').find_element_by_tag_name('h3').find_element_by_tag_name('a').text
                    go_ahead = True
                except:
            ...
            if i%100 == 0:
                if len(certis) > 0:
                    try:
                        df_movie_certis = pd.read_csv(data_folder+'movie_certificates.csv')
                    except:
                        df_movie_certis = pd.DataFrame()

                    df_movie_certis = df_movie_certis.append(certis)
                    df_movie_certis.to_csv(data_folder+'movie_certificates.csv', index=False)
                    del df_movie_certis
                    certis = []
                if len(movie_content) > 0:
                    try:
                        df_movie_content = pd.read_csv(data_folder+'movie_content.csv')
                    except:
                        df_movie_content = pd.DataFrame()

                    df_movie_content = df_movie_content.append(movie_content)
                    df_movie_content.to_csv(data_folder+'movie_content.csv', index=False)
                    del df_movie_content
                    movie_content = []
            i += 1
        else:
            j += 1
    ...
    df = pd.read_csv(data_folder+'movie_certificates.csv')
