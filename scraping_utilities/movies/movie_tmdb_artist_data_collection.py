...
def movie_tmdb_artist_data_collection(titles=None):
    config = yaml.safe_load(open('./../config.yml'))
    data_folder = config['movies_data_folder']
    tmdb_api_key = config['tmdb_api_key']

    try:
        df_crew = pd.read_csv(data_folder+'movie_crew.csv')
        df_crew = pd.concat(
            [df_crew[['person_id']][(df_crew['credit_order'] <= 5) & (df_crew['credit_category'] != 'Cast')],
             df_crew[['person_id']][(df_crew['credit_order'] <= 15) & (df_crew['credit_category'] == 'Cast')]], axis=0)
    ...
    try:
        df_movie_tmdb_artists_already_scraped = pd.read_csv(data_folder + 'movie_tmdb_artists.csv')
        artists_scraped = list(df_movie_tmdb_artists_already_scraped['imdb_id'].unique())
        del df_movie_tmdb_artists_already_scraped
    except:
        artists_scraped = []
    ...
    for artist in artists:
        if artists_scraped.count(artist) == 0:
            ...
            if i%50 == 0:
                ...
                try:
                    df_main = pd.read_csv(data_folder + 'movie_tmdb_artists.csv')
                except:
                ...
                df_main.to_csv(data_folder + 'movie_tmdb_artists.csv', index=False)
                del df_main
                del df
                details = []
        else:
