...
try:
    ...
    df_crew = pd.concat([df_crew[['person_id']][(df_crew['credit_order']<=5) & (df_crew['credit_category'] != 'Cast')],df_crew[['person_id']][(df_crew['credit_order']<=15) & (df_crew['credit_category'] == 'Cast')]], axis=0)
...
for awardee_id in awardees:
    if awardees_scraped.count(awardee_id) == 0:
        ...
        print('Awardee', awardee_id, 'skipped as scraped already')
        print('\n')
        j += 1
