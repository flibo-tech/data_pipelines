...
def stream_online(title):
    ...
    if items:
        for item in items:
            if (item.get('title', item.get('original_title', 'No TV Series name found')).lower() == title[1].lower()) & (item['original_release_year'] == title[2]):
                resp = item
                break
    return resp


df_ratings_n_stream = pd.read_csv('tv_series_ratings_n_stream_30k.csv')
titles_collected = list(df_ratings_n_stream['title_id'].unique())
titles = list(df_tv_series.apply(lambda row: (row['tconst'], row['primaryTitle'], row['startYear']), axis=1).unique())
# titles = list(df_tv_series['tconst'].unique())
titles = titles[:30000]
titles = [title for title in titles if title[0] not in titles_collected]
del df_ratings_n_stream
df = pd.DataFrame()
i = len(titles_collected)
print(i)
for title in titles:
    ...
    if  i%100 == 0:
        ...
        df_ratings_n_stream.to_csv('tv_series_ratings_n_stream_30k.csv', index=False)
        print(i)
        del df_ratings_n_stream
        df = pd.DataFrame()


df = pd.read_csv('./cleaned_data/tv_series_just_watch_30k.csv')
...
df_ratings_n_stream.to_csv('./cleaned_data/tv_series_ratings_n_stream_30k.csv', index=False)