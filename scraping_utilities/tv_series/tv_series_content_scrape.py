...
def tv_series_content_scrape(df_titles):
    import re
    ...
    for title_id in titles:
        ...
        if go_ahead:
            try:
                ...
                tv_series_name = soup.find('div', class_='subpage_title_block').find('h3').find('a').text
    ...
    if not df.empty:
        ...
        df_tv_series_certis = df[['title_id', 'certificate_by', 'rating', 'rating_cleaned', 'age_limit', 'pg', 'banned']]

    df_tv_series_certis['df_type'] = 'certis'
    df_tv_series_content['df_type'] = 'content'

    df = pd.concat([df_tv_series_certis, df_tv_series_content], axis=0)
