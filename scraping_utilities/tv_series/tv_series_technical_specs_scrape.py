...
def tv_series_technical_specs_scrape(titles):
    ...
    try:
        df_tv_series_technical_specs_already_scraped = pd.read_csv(data_folder+'tv_series_technical_specs.csv')
        titles_scraped = list(df_tv_series_technical_specs_already_scraped['title_id'])
        del df_tv_series_technical_specs_already_scraped
    except:
        titles_scraped = []
        ...
        if titles_scraped.count(title_id) == 0:
            ...
            if i%100 == 0:
                if df_main.shape[0] > 0:
                    try:
                        df_tv_series_technical_specs = pd.read_csv(data_folder+'tv_series_technical_specs.csv')
                    except:
                        df_tv_series_technical_specs = pd.DataFrame()

                    df_tv_series_technical_specs = pd.concat([df_tv_series_technical_specs,df_main], axis=0)
                    df_tv_series_technical_specs.to_csv(data_folder+'tv_series_technical_specs.csv', index=False)
                    del df_tv_series_technical_specs
                    df_main = pd.DataFrame()
            i += 1
        else:
            j += 1
