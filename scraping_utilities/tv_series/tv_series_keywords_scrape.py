...
def tv_series_keywords_scrape(titles):
    ...
    try:
        df_tv_series_keywords_already_scraped = pd.read_csv(data_folder+'tv_series_keywords.csv')
        titles_scraped = list(df_tv_series_keywords_already_scraped['title_id'])
        del df_tv_series_keywords_already_scraped
    except:
        titles_scraped = []
        ...
        if titles_scraped.count(title_id) == 0:
            ...
            if i%100 == 0:
                if df_main.shape[0] > 0:
                    try:
                        df_tv_series_keywords = pd.read_csv(data_folder+'tv_series_keywords.csv')
                    except:
                        df_tv_series_keywords = pd.DataFrame()

                    df_tv_series_keywords = pd.concat([df_tv_series_keywords,df_main], axis=0)
                    df_tv_series_keywords.to_csv(data_folder+'tv_series_keywords.csv', index=False)
                    del df_tv_series_keywords
                    df_main = pd.DataFrame()
            i += 1
        else:
            j += 1
    ...
    df = pd.read_csv(data_folder+'tv_series_keywords.csv')
    ...
    df.to_csv(data_folder+'cleaned_tv_series_keywords.csv', index=False)
    return True
